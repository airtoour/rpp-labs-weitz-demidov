...
# 3 ПОЛЯ 2 ЧИСЛОВЫХ 1 ТЕКСТ. 1 ЧИСЛО 2 ЗНАКА ОПЕРАЦИИ + ИЛИ -
class PlusMinusForm(FlaskForm):
    number_one = IntegerField('Число первое', validators=[DataRequired()])
    number_two = IntegerField('Число второе', validators=[DataRequired()])
    operation  = StringField('Операция', validators=[DataRequired()])

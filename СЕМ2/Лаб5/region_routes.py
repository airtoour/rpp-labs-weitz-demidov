...
def add_region():
    ...
    if form.validate_on_submit():
        ...
        if Region.query.filter_by(id=region_code).first():  # Обращаемся к таблице по введенному коду, если он существует в бд выводим надпись
            message = 'Регион с этим кодом уже существует'
        else:
            regions = Region(id=region_code, name=name)  # Иначе создаем новый регион, код региона в айди, имя в имя
        ...
        return render_template('region-add.html', form=form, message=message)  # form=form это мы даем доступ html работать с классами FlaskForm в файле models
...
# В ЦЕЛОМ В ОСТАЛЬНЫХ ФАЙЛАХ ЛОГИКА ОДИНАКОВАЯ, ТАК ЧТО ПОНИМАНИЯ ЭТОГО ФАЙЛА ДОСТАТОЧНО.
# ЗАДАНИЕ НАХОДИТСЯ В TAX_ROUTES В САМОМ НИЗУ